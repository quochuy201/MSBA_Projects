...
/****** Object:  View [dbo].[view_final]    Script Date: 3/16/2020 2:47:14 PM ******/
SET ANSI_NULLS ON
GO
...
select  r.reviewid, r.reviewerid, b.businessid,
		...
		case 
			when t1.rowid =1 then 1
			else 0
			end as firstreview, --- first review indicated by rank of row order by review date.
...
where (year(r.date) between 2010 and 2012) and (flagged != 'NR' and flagged !='YR')
